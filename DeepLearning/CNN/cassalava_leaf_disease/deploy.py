...
model = load_model('DeepLearning\CNN\cassalava_leaf_disease\lenet_model.pt')
...
def main():
    ...
    option = st.selectbox('How would you like to give the inputs?', ('Upload Image File', 'Run Example File'))
    if option == "Upload Image File":
        file = st.file_uploader("Flease upload an image", type=["jpg", "png"])
        if file is not None:
    ...
    elif option=="Run Example File":
        image = Image.open('DeepLearning\CNN\cassalava_leaf_disease\demo_cbsd.jpg')
        p, idx = inference(image, model)
        label = idx2label[idx]
        st.image(image)
